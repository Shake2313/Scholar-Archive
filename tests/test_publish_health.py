...
class PublishHealthTests(unittest.TestCase):
    ...
    def test_check_supabase_publish_health_reports_missing_credentials(self):
        with patch("publish.get_supabase_url", return_value=None), patch(
            "publish.get_supabase_service_key", return_value=None
        ):
    ...
    def test_check_supabase_publish_health_reports_auth_failure(self):
        with patch("publish.socket.getaddrinfo", return_value=[object()]), patch(
            "publish._supabase_request",
    ...
    def test_publish_bundle_to_supabase_stops_after_failed_health_check(self):
        ...
        with patch("publish.check_supabase_publish_health", return_value=health_check), patch(
            "publish._ensure_storage_bucket"
        ) as ensure_bucket:
    ...
    def test_publish_bundle_to_supabase_skips_existing_slug_when_policy_is_skip(self):
        ...
        with patch("publish.check_supabase_publish_health", return_value=health_check), patch(
            "publish._fetch_existing_document_by_slug",
            return_value=existing_document,
        ), patch("publish._ensure_storage_bucket") as ensure_bucket:
    ...
    def test_publish_bundle_to_supabase_overwrites_existing_slug_when_policy_is_overwrite(self):
        ...
        with patch("publish.check_supabase_publish_health", return_value=health_check), patch(
            "publish._fetch_existing_document_by_slug",
            return_value=existing_document,
        ), patch("publish._ensure_storage_bucket"), patch(
            "publish._upsert_rows",
            side_effect=[[{"id": 12, "slug": "demo-paper"}], [], [], []],
        ) as upsert_rows, patch("publish._delete_rows"):
