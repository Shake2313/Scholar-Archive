...
```bash
python -m backend.pipeline --input paper.pdf --name PaperName --output ./output/PaperName
```

게시 없이 실행:

```bash
python -m backend.pipeline --input paper.pdf --name PaperName --output ./output/PaperName --no-publish
```
...
```bash
python -m backend.pipeline --input paper.pdf --name PaperName --output ./output/PaperName --pages 1-3
```

메타데이터만 다시 추론:

```bash
python -m backend.pipeline --name PaperName --output ./output/PaperName --metadata-only
```

한국어 LaTeX만 다시 생성:

```bash
python -m backend.pipeline --name PaperName --output ./output/PaperName --translation-only
```

한국어 PDF만 다시 컴파일:

```bash
python -m backend.pipeline --name PaperName --output ./output/PaperName --korean-pdf-only
```

캐시를 유지하되 이미지만 강제로 다시 렌더링:

```bash
python -m backend.pipeline --input paper.pdf --name PaperName --output ./output/PaperName --refresh-images
```

캐시를 유지하되 메타데이터만 강제로 다시 추론:

```bash
python -m backend.pipeline --input paper.pdf --name PaperName --output ./output/PaperName --refresh-metadata
```
...
- batch publish는 `python -m backend.publish --output-root ./output`로 publish-ready output 폴더만 우선순위대로 순차 처리한다.
- batch publish 전 점검만 하려면 `python -m backend.publish --output-root ./output --dry-run`을 사용한다.

- remote slug 충돌의 기본 정책은 `overwrite`다. 같은 slug가 이미 있으면 같은 문서를 같은 id에 덮어쓴다. 기존 문서를 유지하려면 `--slug-conflict skip`을 사용한다.

- 메타데이터가 빈약한 문서는 `python -m backend.publish --output-dir ./output/PaperName --name PaperName --write-metadata-override --title "..." --author "..." --publication-year 1752`로 수동 보정 파일을 만든다.
...
- 기준 문서: `docs/FRONTEND_ARCHIVE_DESIGN.md`
...
작업표시줄에서 로컬 운영 UI를 실행하려면 `tools/windows/ScholarArchive.exe`를 사용할 수 있다.
...
```bat
tools\windows\build_launcher.bat
```
...
```bash
python -m py_compile backend/pipeline.py backend/publish.py backend/app.py backend/steps.py backend/prompts.py tests/test_helpers.py
```
...
Current priorities and completion history are tracked in [CODEX_CHECKLIST.json](C:/Users/박수인/Desktop/백업/Coding/Scholar%20Archive/CODEX_CHECKLIST.json).
